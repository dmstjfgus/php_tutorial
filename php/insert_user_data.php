...
//   )";
  ...
  // 6. 데이터 값 변경(updata) : UPDATE [table name] SET [field name] = [filed name] WHERE [field name] = [field name]
  // WHERE 필터링 조건 없이 업데이트만 한다면 업데이트 대상 필드의 모든 값이 지정 값으로 변경된다. 따라서 어떠한 데이터를 업데이트 하는지에 대한 필터링이 반드시 있어야 한다.
  // $sql = "UPDATE spl_user SET user_name = '최성은' WHERE user_idx=1"; 

  // 7. 데이터 삭제(delete) : DELETE FROM [table name] WHERE [field name] = [field value];
  $sql = "DELETE FROM spl_user WHERE user_idx = 1 and user_name = '최성은'";
